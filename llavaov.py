...
with torch.inference_mode():
    for i, row in dataset.iterrows():
        ...
        inputs1 = processor(videos=list(video_paths[0]), text=prompt, return_tensors='pt').to(model.device, torch.bfloat16)
        inputs2 = processor(videos=list(video_paths[1]), text=prompt, return_tensors='pt').to(model.device, torch.bfloat16)
        ...
        correct_answer1 = 'A' if 'a' in correct_answer1 else 'B'
        correct_answer2 = 'A' if 'a' in correct_answer2 else 'B'
